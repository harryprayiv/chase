...
let
  compile-manifest = pkgs.writeShellScriptBin "compile-manifest" ''
    ...
    compile_haskell() {
      ...
      (cd "$PROJECT_ROOT" && cabal build all) > "$tmp" 2>&1
      local rc=$?
    ...
    clean_haskell() {
      perl -0777 -pe '
        my @pragmas;
        while ($_ =~ /(\{-#\s*LANGUAGE.*?-\})/gs) { push @pragmas, $1; }
        s/\{-#\s*LANGUAGE.*?-\}//gs;
        s/(\s*)--.*$/$1/gm;
        s/\{-(?!#).*?-\}//gs;
        if (@pragmas) {
    ...
    process_section() {
      ...
      local compile_output="" compile_status="" sfx=""
      if [ -n "$compile_fn" ]; then
        ...
        if echo "$compile_output" | grep -q "^COMPILE_STATUS: false"; then
          sfx="_ERR"; compile_status="FAILED"
...

      {
        ...
        if [ -n "$compile_output" ] && echo "$compile_output" | grep -q "^COMPILE_STATUS: false"; then
          echo "{-"
  ...
  llm-context = pkgs.writeShellScriptBin "llm-context" ''
    ...
    ALL_MANIFEST_CHANGED=$(printf '%s\n%s\n%s\n' \
      "$HS_CHANGED" "$HS_TEST_CHANGED" "$NIX_CHANGED" \
      | sort -u | grep -v '^$' || true)
    ...
    OVERALL_STATUS=""
    if [ "$HS_STATUS" = "_ERR" ]; then
      OVERALL_STATUS="_ERR"
    elif [ "$HS_STATUS" = "_OK" ]; then
      OVERALL_STATUS="_OK"
    fi
...
in {
  inherit compile-manifest compile-archive llm-context;
}